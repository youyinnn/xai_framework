...
from rank_model.s2search.text import find_query_ngrams_in_text, fix_text, STOPWORDS
from rank_model.s2search.text import extract_from_between_quotations, fix_author_text
from rank_model.s2search.text import standardize_whitespace_length
...
def make_feature_names_and_constraints():
    ...
    for field in ['title', 'abstract', 'venue']:
        ...
        feats.extend([
            ...
            f'{field}_frac_of_query_matched_in_text',
    ...
    feats.extend([
        # total amount of (fractional wrt query) matched authors
        'sum_matched_authors_len_divided_by_query_len',
        # largest (fractional wrt query) author match
        'max_matched_authors_len_divided_by_query_len',
        # how far the author matches are from the front/back of the author list
...
def make_features(query, result_paper, lms, max_q_len=128, max_field_len=1024):
    ...
    # total length of quoted snippets
    q_quoted_len = np.sum([len(i) for i in q_quoted])
    # total length of non-quoted snippets
    q_unquoted_len = np.sum([len(i) for i in q_unquoted])
    ...
    feats = [
        result_paper['paper_abstract_cleaned'] is not None and len(
            result_paper['paper_abstract_cleaned']) > 1,
    ...
    for field in ['paper_title_cleaned', 'paper_abstract_cleaned', 'paper_venue_cleaned']:
        ...
        # unquoted matches
        unquoted_match_spans, unquoted_match_text, unquoted_longest_starting_ngram = find_query_ngrams_in_text(
            q_unquoted, text, quotes=False)
        unquoted_matched_across_fields.extend(unquoted_match_text)
        unquoted_match_len = len(unquoted_match_spans)

        # quoted matches
        quoted_match_spans, quoted_match_text, quoted_longest_starting_ngram = find_query_ngrams_in_text(
            q_quoted, text, quotes=True)
        ...
        if 'venue' in field:
            ...
            for sp, tx in zip(match_spans_set, match_text_set):
                ...
                condition_1 = (tx_unigrams.intersection(
                    title_and_abstract_matches) == tx_unigrams)
        ...
        if len(match_text_set) > 0 and text_len > 0:
            # log probabilities of the scores
            if 'venue' in field:
                lm_probs = [lm_score(match, 'venue')
                            for match in match_text_set]
            else:
            ...
            match_spans_len_normed = np.log1p(
                list(match_text_counter.values())).sum()
    ...
    q_split_on_quotes = [i.strip()
                         for i in q_auth.split('"') if len(i.strip()) > 0]
    ...
    q_quoted_auth = [remove_unigrams(
        i, title_and_venue_matches) for i in q_quoted_auth]
    q_unquoted_auth = [remove_unigrams(
        i, title_and_venue_matches) for i in q_unquoted_auth]
    ...
    for paper_author in authors:
        ...
        if len_author > 0:
            ...
            # last name is ten times more important to match
            paper_author_weights[-len_last_name:] *= 10
            ...
            for quotes_flag, q_loop in zip([False, True], [q_unquoted_auth, q_quoted_auth]):
                matched_spans, match_text, _ = find_query_ngrams_in_text(
                    ...
                    quotes=quotes_flag,
                    len_filter=0,
                    # only removes entire matches that are stopwords. too bad for people named 'the' or 'less'
                ...
                if len(matched_spans) > 0:
                    ...
                    if len(matched_text_joined) == 1:
                        matched_text_joined = ''
                    weight = np.sum([paper_author_weights[i:j].sum()
                                    for i, j in matched_spans])
                    match_frac = np.minimum(
                        (len(matched_text_joined) / q_len), 1)
                    match_fracs.append(match_frac)
                    if quotes_flag:
                        quoted_match_lens.append(match_frac * weight)
                        quoted_matched_across_fields.append(
                            matched_text_joined)
                    else:
                        unquoted_match_lens.append(match_frac * weight)
                        unquoted_matched_across_fields.append(
                            matched_text_joined)
                else:
                    if quotes_flag:
    ...
    feats.extend([
        # total amount of (weighted) matched authors
        np.nansum(match_lens_max),
        # largest (weighted) author match
        nanwrapper(np.nanmax, match_lens_max),
    ...
    feats.extend([
        ...
        np.nan if np.isnan(
            year) else result_paper['n_citations'] / (now.year - year + 1)
    ])
    ...
    if len(q_unquoted_split_set) > 0:
        ...
        log_probs_unmatched_unquoted = [
            lm_score(i, 'max') for i in unmatched_unquoted]
        feats.append(
            np.nansum([i for i in log_probs_unmatched_unquoted if i > log_prob_nonsense]))
    else:
...
author_match_ind = feature_names.index(
    'max_matched_authors_len_divided_by_query_len')
matched_all_ind = feature_names.index(
    'fraction_of_unquoted_query_matched_across_all_fields')
title_match_ind = feature_names.index('title_frac_of_query_matched_in_text')
abstract_match_ind = feature_names.index(
    'abstract_frac_of_query_matched_in_text')
...
def posthoc_score_adjust(scores, X, query=None):
    ...
    scores[has_quotes_to_match] += 1000 * \
        quotes_frac_found[has_quotes_to_match]
    ...
    if query_len > 1:
        if '"' in query:
            qualifying_for_cutoff = np.isclose(
                X[:, quotes_feat_ind], 1.0) & matched_all_flag
        else:
        ...
        if len(where_zeros[0]) > 0:
            ...
            if top_cutoff > 1:
                ...
                pattern_of_matches = 1000 * ((X[top_inds, title_match_ind] > 0) | (X[top_inds, abstract_match_ind] > 0)) + \
                    ...
                    year_match[top_inds]
                most_common_pattern = Counter(
                    pattern_of_matches).most_common()[0][0]
                ...
                if most_common_pattern != 1000:
                    scores[top_inds[pattern_of_matches ==
                                    most_common_pattern]] += 10000
